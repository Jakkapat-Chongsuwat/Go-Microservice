...
export class HelmChartPipeline extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    const packageAndPushChart = new command.local.Command(
      `${name}-package-and-push`,
      {
        create: pulumi.interpolate`
                ...
                helm registry login --username AWS --password-stdin ${callerIdentity.then(
                  (id) => `${id.accountId}.dkr.ecr.${args.region}.amazonaws.com`
                )} && \
                helm push ${args.chartName}-${args.chartVersion}.tgz \
                oci://${callerIdentity.then(
                  (id) =>
                    `${id.accountId}.dkr.ecr.${args.region}.amazonaws.com/${args.ecrRepositoryName}`
                )}
            `,
        environment: {
    ...
    if (args.deploy !== false) {
      const chartUrl = pulumi.interpolate`oci://${callerIdentity.then(
        (id) =>
          `${id.accountId}.dkr.ecr.${args.region}.amazonaws.com/${args.ecrRepositoryName}/${args.chartName}`
      )}`;
      ...
      this.helmDeployment = new HelmChart(
        `${name}-deployment`,
        {
          ...
          values: args.values || {},
          repositoryOpts: {
            repository: pulumi.interpolate`${callerIdentity.then(
              (id) =>
                `${id.accountId}.dkr.ecr.${args.region}.amazonaws.com/${args.ecrRepositoryName}`
            )}`,
          },
